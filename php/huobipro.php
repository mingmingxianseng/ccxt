...
class huobipro extends Exchange
{
    ...
    public function parse_ticker($ticker, $market = null)
    {
        ...
        if (is_array($ticker) && array_key_exists('bid', $ticker)) {
            if (gettype($ticker['bid']) === 'array'
                && count(array_filter(array_keys($ticker['bid']), 'is_string')) == 0) {
                $bid       = $this->safe_float($ticker['bid'], 0);
                $bidVolume = $this->safe_float($ticker['bid'], 1);
            }
        }
        if (is_array($ticker) && array_key_exists('ask', $ticker)) {
            if (gettype($ticker['ask']) === 'array'
                && count(array_filter(array_keys($ticker['ask']), 'is_string')) == 0) {
    ...
    public function fetch_ticker($symbol, $params = array())
    {
        $this->load_markets();
        $market   = $this->market($symbol);
        $response = $this->marketGetDetailMerged(
            array_merge(
                array(
                    'symbol' => $market['id'],
                ), $params
    ...
    {
        $response   = $this->publicGetSettingsCurrencys(
            array_merge(
                array(
    ...
    {
        return $this->fetch_orders_by_states(
    ...
    {
        $this->load_markets();
        $response = $this->privateGetOrderOrdersId(
            array_merge(
                array(
                    'id' => $id,
                ), $params
            ...
            if (($type === 'market') && ($side === 'buy')) {
                if ($price === null) {
                    throw new InvalidOrder (
                        $this->id
                        . " $market buy order requires $price argument to calculate cost (total $amount of quote currency to spend for buying, $amount * $price). To switch off this warning exception and specify cost in the $amount argument, set .options['createMarketBuyOrderRequiresPrice'] = false. Make sure you know what you're doing."
                    );
                } else {
    ...
    public function fetch_deposit_address($code, $params = array())
    {
        ...
        $response = $this->privateGetDwDepositVirtualAddresses(
            array_merge(
                array(
